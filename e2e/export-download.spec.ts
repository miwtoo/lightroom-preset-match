...
test('export download flow: valid preset name triggers .xmp download and shows iPad import guidance', async ({ page }) => {
  ...
  await test.step('Then the export panel is visible', async () => {
    await expect(page.getByRole('heading', { name: 'Export Preset (.xmp)' })).toBeVisible()
  })
  ...
  await test.step('And Download .xmp Preset is clicked', async () => {
    const downloadPromise = page.waitForEvent('download')
    await page.getByRole('button', { name: 'Download .xmp Preset' }).click()
